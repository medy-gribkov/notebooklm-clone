...
export function ChatInterface({ notebookId, initialMessages, isProcessing = false, hasFiles = true, hasErrorFiles = false, description, starterPrompts: dynamicPrompts, onFileUploaded, isUploading: externalUploading, setIsUploading }: ChatInterfaceProps) {
  ...
  const priorMessages: UIMessage[] = initialMessages.map((m) => ({
    id: m.id,
  ...
  const transport = useMemo(
    () => new DefaultChatTransport({ api: "/api/chat", body: { notebookId } }),
    [notebookId],
  );
        ...
        <div className="px-4 py-6">
          <div className="space-y-5 max-w-2xl lg:max-w-3xl mx-auto">
            ...
            {messages.map((message: UIMessage) => {
              ...
              const sources: Source[] | undefined = sourcesById[message.id];
                    ...
                    >
                      ...
                      >
                        {isUser ? (
                          ...
                          <MarkdownRenderer content={textContent} sources={sources} />
                        )}
                        ...
                        {!isUser && textContent && (
                          <div className="absolute top-2 end-2 flex items-center gap-0.5 opacity-100 sm:opacity-0 sm:group-hover/msg:opacity-100 transition-opacity z-10">
                            ...
                            <button
                              onClick={() => saveToNote(message.id, textContent)}
                              className="p-1.5 rounded-md hover:bg-background/80 text-muted-foreground/50 hover:text-foreground transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30"
