...
const { mockStreamText, mockStreamData } = vi.hoisted(() => {
  const mockStreamData = vi.fn(function() {
    return {
      append: vi.fn(),
      close: vi.fn(),
    };
  ...
  const mockStreamText = vi.fn(() => ({
    toDataStreamResponse: vi.fn(() => new NextResponse("stream", { status: 200, headers: { "Content-Type": "text/plain; charset=utf-8" } })),
  }));

  return { mockStreamText, mockStreamData };
});
