...
export async function DELETE(request: Request) {
  ...
  if (!checkRateLimit(`${user.id}:delete-account`, 1, 86_400_000)) {
  ...
  try {
    ...
    // 4f. Delete companies (companies table has notebook_id, not user_id)
    if (notebookIds.length > 0) {
      await serviceClient.from("companies").delete().in("notebook_id", notebookIds);
    }
